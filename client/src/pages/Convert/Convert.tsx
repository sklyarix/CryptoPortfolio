...
const Convert = () => {
	...
	const onChangeSendValue = (valueInput: string) => {
		...
		return setReceiveValue(
			String(
				(+valueInput * getCoin(sendCurrency).price) /
					getCoin(receiveCurrency).price
			)
		)
	}
	const onChangeReceiveValue = (valueInput: string) => {
		...
		return setSendValue(
			String(
				(+valueInput * getCoin(receiveCurrency).price) /
					getCoin(sendCurrency).price
			)
		)
	}
			...
			<div className={styles.wrapper}>
				<div className={styles.item}>
					...
					<div className={styles['item-bottom']}>
						<InputConvert value={sendValue} setValue={onChangeSendValue} />
						<SelectConvert
							value={sendCurrency}
